			 ...
			 Tl* prev_h, Tl* new_h, bool print_board = false)
{
	...
	for(int turn=0; turn<turns; turn++){
		...
		for(int i=0; i<prev_h->neigh_changes_num[turn]; ++i){
			...
			neigh_count_hist[prev_h->neigh_changes_coords[turn][i][0]][prev_h->neigh_changes_coords[turn][i][1]][prev_h->neigh_changes[turn][i][0]] = prev_h->neigh_changes[turn][i][1];
		}
		...
		Coords prev_ctp;
		memcpy(prev_ctp,cells_to_process,sizeof(int)*2*prev_ctp_num);
		...
		if(new_h != NULL){
			...
			for(int i=0; i<prev_ctp_num; ++i){
				...
				col = prev_ctp[i][1];
				if(!cell_added[row][col]){
					int p1_neigh_cnt = neigh_count_hist[row][col][0];
					new_h->neigh_changes_coords[turn][neigh_changes_num][0] = row;
					new_h->neigh_changes_coords[turn][neigh_changes_num][1] = col;
					new_h->neigh_changes[turn][neigh_changes_num][0] = player_1;
					new_h->neigh_changes[turn][neigh_changes_num][1] = p1_neigh_cnt;
					...
					int p2_neigh_cnt = neigh_count_hist[row][col][1];
					new_h->neigh_changes_coords[turn][neigh_changes_num][0] = row;
					new_h->neigh_changes_coords[turn][neigh_changes_num][1] = col;
					new_h->neigh_changes[turn][neigh_changes_num][0] = player_2;
					new_h->neigh_changes[turn][neigh_changes_num][1] = p2_neigh_cnt;
		...
		if(changed_cells_num == 0){
			...
			if(new_h != NULL){
				for(int t=turn+1; t<TURNS_NUM; ++t){
					...
					// neigh count
					memcpy(new_h->neigh_changes[t],prev_h->neigh_changes[t],sizeof(int)*2*prev_h->neigh_changes_num[t]);
					memcpy(new_h->neigh_changes_coords[t],prev_h->neigh_changes_coords[t],sizeof(int)*2*prev_h->neigh_changes_num[t]);
					
				}
				memcpy(&(new_h->changes_num[turn+1]),&(prev_h->changes_num[turn+1]),sizeof(int)*(TURNS_NUM-turn-1));
				memcpy(&(new_h->neigh_changes_num[turn+1]),&(prev_h->neigh_changes_num[turn+1]),sizeof(int)*(TURNS_NUM-turn-1));
